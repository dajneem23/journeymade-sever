...
export default class VolumeService {
  private volumeSQL: string;
  constructor(
    @Inject('volumeModel') private volumeModel: Models.VolumeModel,
    @Inject('logger') private logger,
    @EventDispatcher() private eventDispatcher: EventDispatcherInterface,
  ) {
    this.volumeSQL = fs.readFileSync(__dirname + '/queries/volume.sql').toString();
  }
  ...
  public async getVolumeFromPgDB({
    token_id,
    period,
    limit,
    to_time,
    ...
    // sql = sql.replaceAll('$6', `'${minTime}'`);
    
    const fromTime = timeFrames[0][0];
    const toTime = timeFrames[timeFrames.length - 1][1];
    sql = sql.replaceAll('$4', `'${dayjs.unix(fromTime).format('YYYY-MM-DD HH:mm:ss')}'`);
    sql = sql.replaceAll('$5', `'${dayjs.unix(toTime).format('YYYY-MM-DD HH:mm:ss')}'`);

    // console.log("🚀 ~ file: volume.ts:129 ~ VolumeService ~ sql:", sql)
    // const query = `SELECT * from get_volume_by_token_id('${token_id}', '${period}')`;
