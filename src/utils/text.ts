 ...
 * @returns - parsed url
 * @example - https://twitter.com/naglinagli/status/1598268226990809088?s=20&t=zbE0leoquMsVyu8CwgEkPQ => 1598268226990809088
 */
...
export const getTwitterStatusIdFromUrl = (url: string) => {
  const regex = /https?:\/\/twitter.com\/\w+\/status\/(\d+)/;
  const match = url.match(regex);
  return match ? match[1] : null;
};
export const getTwitterUserIdFromUrl = (url: string) => {
