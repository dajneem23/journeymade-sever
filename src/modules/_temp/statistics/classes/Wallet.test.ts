...
describe('Wallet', () => {
  ...
  it('should return value', () => {
    const address = '0xcb5cea9e0000e02552f520f401d1f6793814c1e2';
    const addressPortfolios = {};
    Object.keys(portfolios).forEach((key) => {
      addressPortfolios[key] = portfolios[key].filter(
        (p) => p.address === address,
      );
    });

    const periodOptions = prepareCrawlIds({ crawl_id: 2023022001 });

    const wallet = new Wallet({
      address,
      portfolios: addressPortfolios,
    ...
    expect(result.statistics[2]).toEqual({
      period: 6,
      amount: 711401.603,
      usd_value: 1067003.227,
      price: 1.499,
      from_time: 1676826600,
      to_time: 1676832600,
      percentage_change: 0.006,
      usd_percentage_change: -0.652,
      chains: 'matic,eth',
      pool_adapter_ids: 'uniswap2_liquidity_proxy,polygon_staked',
    });
  });
});
